...
export default function TemplateViewPage() {
  ...
  const handleDeleteDay = async (sessionId: number) => {
    if (!confirm("¿Estás seguro de eliminar este día y todos sus ejercicios?")) {
      return;
    }

    try {
      await deleteDayFromPlan(sessionId);
    ...
    <div className="min-h-screen bg-zinc-950 p-4 md:p-6">
      <div className="mb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-zinc-100 uppercase tracking-tight">{template.name}</h1>
      ...
      <div className="space-y-6">
        {template.sessions && template.sessions.length > 0 ? (
                    ...
                    {(sessions as any[]).map((session: any) => (
                      <div key={session.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-4 mb-4">
                          ...
                          <button
                            onClick={() => handleDeleteDay(session.id)}
                            className="flex items-center gap-1.5 rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-2 text-xs font-black text-red-400 transition hover:bg-red-500/20 hover:border-red-400"
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                            Eliminar Día
                        ...
                        {session.session_exercises && session.session_exercises.length > 0 ? (
                          <ExerciseExcelGrid exercises={session.session_exercises} role="COACH" isTemplate={true} />
                        ) : (
          ...
          <div className="text-center py-24 rounded-xl border-2 border-dashed border-zinc-800">
            ...
            <p className="text-zinc-500 text-sm">Esta plantilla no tiene sesiones configuradas</p>
          </div>
        )}
      </div>
    </div>
